...
class App extends Component {
  ...
  savePlaylist() {
    ...
    this.spotifyClient.savePlaylist(this.state.playlistName, uriArray);
    this.setState({ playlistTracks: [], playlistName: 'New Playlist' });
  }

  search(term) {
    this.spotifyClient.search(term).then(results => Array.from(results)).then(trackArray => {this.setState({ searchResults: trackArray })});
  }
