...
export default class Spotify {
  getAccessToken() {
    if (accessToken) {
      ...
      if (temp) {
        ...
        let redirectURI = encodeURI(apiURL);
  ...
  search(term) {
    ...
    let authHeader =  { headers: { Authorization: `Bearer ${accessToken}` } };
    const searchUrl = `${apiURL}/search?type=track&q=${term}`;
    return fetch(searchUrl, authHeader).then(response => response.json()).then(jsonResponse => {
